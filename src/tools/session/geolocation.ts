...
export default function geolocation(server: FastMCP): void {
  server.addTool({
    ...
    execute: async (args: GeolocationArgs, _context: Record<string, unknown> | undefined): Promise<ContentResult> => {
      try {
        switch (args.action) {
          case 'get':
        ...
        return errorResult(`Failed to ${args.action} geolocation. Error: ${toolErrorMessage(err)}`);
      }
    },
  });
}
