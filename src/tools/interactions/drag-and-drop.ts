...
async function performAndroidDragAndDrop(
  driver: any,
  startX: number,
  startY: number,
  endX: number,
...
async function performiOSDragAndDrop(
  driver: any,
  startX: number,
  startY: number,
  endX: number,
  ...
  server.addTool({
    ...
    execute: async (args: any, context: any): Promise<any> => {
      ...
      try {
        ...
        if (
          args.sourceElementUUID &&
          typeof args.sourceElementUUID === 'string' &&
          args.sourceElementUUID.trim().length > 0
        ) {
        ...
        if (
          args.targetElementUUID &&
          typeof args.targetElementUUID === 'string' &&
          args.targetElementUUID.trim().length > 0
        ) {
        ...
        if (platform === 'Android') {
          await performAndroidDragAndDrop(
            driver,
            startX,
            startY,
            endX,
