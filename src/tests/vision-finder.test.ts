...
function jsonBBoxResponse(target: string, bbox: [number, number, number, number]): string {
...
describe('AIVisionFinder', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(imageUtil, 'requireSharp').mockReturnValue(((_input: Buffer) => mockSharpInstance) as any);
  ...
  describe('findElement – JSON bbox format (normalized coordinates)', () => {
    test('should return correct bbox, center and target for a typical search button', async () => {
      ...
      fetchSpy.mockImplementation(mockModelResponse(jsonBBoxResponse('Search', normalizedBBox)) as any);

      const {AIVisionFinder} = await import('../ai-finder/vision-finder.js');
      const finder = new AIVisionFinder();

      const result = await finder.findElement(BENCHMARK_IMAGE_BASE64, 'Search button', IMAGE_WIDTH, IMAGE_HEIGHT);
    ...
    test('should call fetch with correct endpoint and authorization header', async () => {
      ...
      fetchSpy.mockImplementation(mockModelResponse(jsonBBoxResponse('Button', normalizedBBox)) as any);
      ...
      const [url, config] = fetchSpy.mock.calls[0] as [string, {headers: Record<string, string>}];
    ...
    test('should send image as JPEG base64 data URL in the request body', async () => {
      const normalizedBBox: [number, number, number, number] = [100, 100, 300, 200];
      fetchSpy.mockImplementation(mockModelResponse(jsonBBoxResponse('Icon', normalizedBBox)) as any);
      ...
      const [_url, requestInit] = fetchSpy.mock.calls[0] as [string, RequestInit];
  ...
  describe('findElement – array bbox format (normalized coordinates)', () => {
    test('should parse array format bbox and return correct result', async () => {
      ...
      fetchSpy.mockImplementation(mockModelResponse(arrayBBoxResponse(normalizedBBox)) as any);

      const {AIVisionFinder} = await import('../ai-finder/vision-finder.js');
      const finder = new AIVisionFinder();

      const result = await finder.findElement(BENCHMARK_IMAGE_BASE64, 'Login button', IMAGE_WIDTH, IMAGE_HEIGHT);
  ...
  describe('findElement – absolute coordinate mode', () => {
    test('should use absolute pixel coordinates directly without conversion', async () => {
      ...
      fetchSpy.mockImplementation(mockModelResponse(jsonBBoxResponse('Submit', absoluteBBox)) as any);

      const {AIVisionFinder} = await import('../ai-finder/vision-finder.js');
      const finder = new AIVisionFinder();

      const result = await finder.findElement(BENCHMARK_IMAGE_BASE64, 'Submit button', IMAGE_WIDTH, IMAGE_HEIGHT);
  ...
  describe('findElement – coordinate boundary clamping', () => {
    test('should clamp coordinates that exceed image boundaries (absolute mode)', async () => {
      ...
      fetchSpy.mockImplementation(mockModelResponse(jsonBBoxResponse('Full screen', outOfBoundsBBox)) as any);

      const {AIVisionFinder} = await import('../ai-finder/vision-finder.js');
      const finder = new AIVisionFinder();

      const result = await finder.findElement(BENCHMARK_IMAGE_BASE64, 'Full screen element', IMAGE_WIDTH, IMAGE_HEIGHT);
  ...
  describe('findElement – coordinate order correction', () => {
    test('should swap x1/x2 when x1 > x2 (absolute mode)', async () => {
      ...
      fetchSpy.mockImplementation(mockModelResponse(jsonBBoxResponse('Element', reversedXBBox)) as any);

      const {AIVisionFinder} = await import('../ai-finder/vision-finder.js');
      const finder = new AIVisionFinder();

      const result = await finder.findElement(BENCHMARK_IMAGE_BASE64, 'Some element', IMAGE_WIDTH, IMAGE_HEIGHT);
    ...
    test('should swap y1/y2 when y1 > y2 (absolute mode)', async () => {
      ...
      fetchSpy.mockImplementation(mockModelResponse(jsonBBoxResponse('Element', reversedYBBox)) as any);
  ...
  describe('findElement – unparseable model response', () => {
    test('should throw when model response contains no valid bbox', async () => {
      fetchSpy.mockImplementation(mockModelResponse('I cannot find the element you are looking for.') as any);
  ...
  describe('findElement – image compression', () => {
    test('should compress image when width exceeds imageMaxWidth and still return valid result', async () => {
      ...
      const normalizedBBox: [number, number, number, number] = [100, 100, 300, 200];
      fetchSpy.mockResolvedValue(buildFetchResponse(jsonBBoxResponse('Button', normalizedBBox)) as any);
  ...
  describe('findElement – invalid bbox after coordinate processing', () => {
    test('should throw when bbox has zero width after clamping (x1 === x2, absolute mode)', async () => {
      ...
      fetchSpy.mockImplementation(mockModelResponse(jsonBBoxResponse('Edge', zeroWidthBBox)) as any);
