...
| `appium_mobile_device_lock` | Lock or unlock the device in one tool (`action`: `lock` or `unlock`). Optional `seconds` for timed lock (`mobile: lock` / `mobile: unlock`). Android and iOS. |
